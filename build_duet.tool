...
imgbuild() {
  ...
  ImageTool ${arch} Rebase 0x10000 "${BUILD_DIR_ARCH}/EfiLoader.efi" \
    "${BUILD_DIR_ARCH}/EfiLoaderRebased.efi" || exit 1
...
if [ "${INTREE}" != "" ]; then
  ...
  src=$(curl -Lfs https://raw.githubusercontent.com/acidanthera/ocbuild/SecurePeAndImageTool/efibuild.sh) && eval "$src" || exit 1
fi
